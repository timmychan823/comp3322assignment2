...
window.onload = () => {
    ...
    .then(response => {
        response.json().then(WR => {
            ...
            document.querySelector("#LocationTemperature h2").addEventListener("click",adjustLocationTemp)
            ...
            document.querySelector("#LocationRainfall h2").addEventListener("click",adjustLocationRainfall)
    ...
    .then(response => {
        response.json().then(AQ => {
            ...
            document.querySelector("#LocationAirQuality h2").addEventListener("click",adjustLocationAirQuality)
                    ...
                    if (MyDistrict){
                        for (x in rainfall_stations){
                            if (MyDistrict.includes(x) || x.includes(MyDistrict)){
                                ...
                                break
                            }else if (MyDistrict == "Central and Western District" && x == "Central & Western District") {
                                document.querySelector("#MyDataRainfall").innerHTML = `<p>${rainfall_stations[x]}<sub>mm</sub></p><img src = "images/rain-48.png"></img>`
                                break
